...
int main()
{
    ...
    BaseClass_pointer = &DerivedClass_object; // BaseClass_pointer is pointing to the object of derived class

    BaseClass_pointer->var_base = 23;
    BaseClass_pointer->display();

    // BaseClass_pointer->var_derived = 32; // BaseClass_pointer will not assign value 32 to var_derived even if the pointer is pointing to DerivedClass_object because the pointer is of type BaseClass so it'll bind only to the BaseClass members ,not to the DerivedClass members. So this line will throw error.
    // BaseClass_pointer->display();

    DerivedClass *DerivedClass_pointer;
    DerivedClass_pointer = &DerivedClass_object; // this pointer will only bind to DerivedClass.
    DerivedClass_pointer->var_base = 24; // DerivedClass_pointer can access var_base because DerivedClass has inherited the BaseClass.
    DerivedClass_pointer->var_derived = 32;
